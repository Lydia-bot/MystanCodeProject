...
def get_pixel_dist(pixel, red, green, blue):
    ...
    """
    # the color's rgb distance formula
...
def get_best_pixel(pixels):
    ...
    # a pixel is the closest to original one
    for i in range(len(pixels)):
        dist = get_pixel_dist(pixels[i], avg[0], avg[1], avg[2])
        # add a tuple in the list
        dist_list.append((dist, pixels[i]))
    ...
    for x in range(result.width):
        for y in range(result.height):
            ...
            for img in images:
                ...
                get_img_pix.append(img_pix)
            pixel = get_best_pixel(get_img_pix)
